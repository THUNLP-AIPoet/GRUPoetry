    ...
    def build_decoder(self, c, y,
        ...
        for level in range(self.num_levels):
            ...
            if self.state['search']:
                if self.compute_alignment:
                    ...
                    h, ctx, alignment, reseter, updater = result
                    if mode == Decoder.EVALUATION:
                        alignment = alignment.out
                else:
                    #This implicitly wraps each element of result.out with a Layer to keep track of the parameters.
                    #It is equivalent to h=result[0], ctx=result[1]
                    h, ctx, reseter, updater  = result
            else:
                h = result
                if mode == Decoder.EVALUATION:
                    ctx = c
        ...
        if mode == Decoder.SAMPLING:
            ...
            return self.output_layer(
                    state_below=readout.out,
                    temp=T).out, alignment, reseter, updater
...
class RNNEncoderDecoder(object):
    ...
    def __init__(self, state, rng,
        ...
        self.rng = rng
        self.skip_init = skip_init
        self.compute_alignment = True#compute_alignment
    ...
    def build(self):
        ...
        forward_results = self.encoder.build_encoder(
                self.sampling_x,
                return_hidden_layers=True)

        self.forward_sampling_c , self.forward_reseter, self.forward_updater = forward_results

        self.forward_reseter = self.forward_reseter.out
        self.forward_updater = self.forward_updater.out

        self.backward_sampling_c, self.backward_reseter, self.backward_updater = self.backward_encoder.build_encoder(
                self.sampling_x[::-1],
                approx_embeddings=self.encoder.approx_embedder(self.sampling_x[::-1]),
                return_hidden_layers=True).out

        self.backward_sampling_c = self.backward_sampling_c[::-1]
        self.backward_reseter = self.backward_reseter[::-1]
        self.backward_updater  = self.backward_updater [::-1]

        #self.backword_reseter = self.forward_reseter.out
        #self.backward_updater = self.forward_updater.out
    ...
    def create_representation_computer(self):
        if not hasattr(self, "repr_fn"):
            self.repr_fn = theano.function(
                    ...
                    outputs=[self.sampling_c, self.forward_reseter, self.forward_updater, self.backward_reseter, self.backward_updater],
                    name="repr_fn")
        return self.repr_fn
    ...
    def create_next_probs_computer(self):
        if not hasattr(self, 'next_probs_fn'):
            probs, alignment, reseter, updater = self.decoder.build_next_probs_predictor(
            ...
            self.next_probs_fn = theano.function(
                    ...
                    outputs=[probs, alignment, reseter, updater],
                    name="next_probs_fn")
