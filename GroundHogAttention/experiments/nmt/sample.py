    ...
    def search(self, seq, n_samples, ignore_unk=False, minlen=1):
        ...
        for k in range(3 * len(seq)):
            ...
            for i, (orig_idx, next_word, next_cost) in enumerate(
                ...
                #print ("%d, %d, %d"%(i, orig_idx, next_word))
        ...
        for i in range(0, len(index)):
            fin_align.append(align[index[i]])

        fin_trans = numpy.array(fin_trans)[numpy.argsort(fin_costs)]
        fin_costs = numpy.array(sorted(fin_costs))
        return fin_trans, fin_costs, align
...
def sample(lm_model, seq, n_samples,
    ...
    if beam_search:
        sentences = []
        trans, costs, align = beam_search.search(seq, n_samples,
        ...
        for i in range(len(costs)):
            if verbose:
                print "{} {}: {}".format(i, costs[i], sentences[i])
