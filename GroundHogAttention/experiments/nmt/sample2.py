...
class BeamSearch(object):
    ...
    def search(self, seq, n_samples, ignore_unk=False, minlen=1):
        ...
        #print c.shape	
        #print len(c[7])
        #print "c shape %d  %d" % (c.shape[0], c.shape[1])
        #print "size of c %s" % (size(c))
        ...
        c_avg = numpy.zeros(c[0].shape)
        for i in range(0, c.shape[0]):
            c_avg += c[i]

        c_avg /= c.shape[0]
        
        return c[0], c[c.shape[0]-1], c_avg, fin_trans, fin_costs
...
def sample(lm_model, seq, n_samples,
    ...
    if beam_search:
        sentences = []
        repvec_head, repvec_tail, repvec_avg, trans, costs = beam_search.search(seq, n_samples,
        ...
        return repvec_head, repvec_tail, repvec_avg, sentences, costs, trans
    elif sampler:
        sentences = []
        all_probs = []
        costs = []
...
def main():
    ...
    if args.source and args.trans:
        ...
        for i, line in enumerate(fsrc):
            ...
            repvec_head, repvec_tail, repvec_avg, trans, costs, _ = sample(lm_model, seq, n_samples, sampler=sampler,
                    beam_search=beam_search, ignore_unk=args.ignore_unk, normalize=args.normalize)
            
            repMatrix_head.append(repvec_head)
            repMatrix_tail.append(repvec_tail)
            repMatrix_avg.append(repvec_avg)
            
            if len(costs) == 0:
                continue
            if (i + 1)  % 1000 == 0:
                logger.debug("Current speed is {} per sentence, now {} sentences".
                        format((time.time() - start_time) / (i + 1), i))
        print "Total cost of the translations: {}".format(total_cost)

        fsrc.close()
        ftrans.close()

        numpy.save("poemvector_head.npy",repMatrix_head)     
        numpy.save("poemvector_tail.npy",repMatrix_tail)      
        numpy.save("poemvector_avg.npy",repMatrix_avg)          
